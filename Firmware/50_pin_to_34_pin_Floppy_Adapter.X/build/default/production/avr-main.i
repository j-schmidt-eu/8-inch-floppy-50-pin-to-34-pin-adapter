...
__asm("\tpsect eeprom_data,class=EEDATA,noexec"); __asm("\tdb\t" "0" "," "0" "," "0" "," "0" "," "76" "," "0" "," "0" "," "0");
...
int main(void)
{
    ...
    static enum HeadLoadMode headLoadMode = HL_FromMotorBEnable;
    static enum ReadyDiskChangeMode readyDiskChangeMode = RDC_FromReady;
    static enum TG43Mode tg43Mode = TG_Write_Active_44_XX;
    static enum MenuMode menuMode = M_Hidden;
    static enum DisplayMode startDisplayMode = D_Track;
    static enum DisplayMode displayMode = D_Track;
    ...
    while (1)
    {
        if (!PORTBbits.RB0)
        {
...

            {
                ...
                case TG_Read_Active_60_XX:
                    tg43 = PORTBbits.RB4 && track >= 60;
                    break;
                case TG_Write_Active_44_XX_Read_Active_60_XX:
                    tg43 = (!PORTBbits.RB4 ? track >= 44 : track >= 60);
                    break;
...

            {
                ...
                PORTAbits.RA7 = !headLoad;
            }


            {
                ready = !PORTAbits.RA2;
                diskChange = !PORTAbits.RA6;
                ...
                if (cmdSwitch)
                {
                    ...
                    else
                    {
                        if(!cmdSwitchLong)
                        {
                            ...
                            else if(!cmdSwitchShort && counts >= 25)
                            {
                                cmdSwitchShort = 1;
                            }
                        }
                    ...
                    else if (cmdSwitchShort)
                    {
                        if (menuMode != M_Hidden)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                case M_TG43:
                                    if(tg43Mode == TG_FromExternalSignal)
                                        tg43Mode = TG_Write_Active_44_XX;
                                    else
                                        tg43Mode++;
                                    break;
                                case M_HeadLoad:
                                    if(headLoadMode == HL_FromExternalSignal)
                                        headLoadMode = HL_FromMotorBEnable;
                                    else
                                        headLoadMode++;
                                    break;
                                case M_ReadyDiskChange:
                                    if(readyDiskChangeMode == RDC_FromExternalSignal)
                                        readyDiskChangeMode = RDC_FromReady;
                                    else
                                        readyDiskChangeMode++;
                                    break;
                                case M_Display:
                                    if(startDisplayMode == D_Dark)
                                        startDisplayMode = D_Track;
                                    else
                                        startDisplayMode++;
                                    break;
                                case M_MaxTrack:
...

        {
            ...
            else
            {
                ...
                case D_Revision:
                    bcd1 = (uint_fast8_t) (01 / 10);
                    bcd2 = (uint_fast8_t) (01 % 10);
                    dot1 = blink;
                    dot2 = !blink;
                    break;
...
void __attribute__((picinterrupt(("low_priority")))) isrLowPrio(void)
{
    ...
    if (PIR1bits.TMR2IF)
    {
        ...
        if(counter6000Hz < ((0xffffffffu) - 1))
            counter6000Hz++;
    }
    ...
    if (PIR4bits.CCP3IF)
    {
        ...
        if (!PORTBbits.RB0 && !PORTBbits.RB5)
        {
            ...
            char sign = 1004 > counter6000Hz;
            uint_fast16_t deviation = sign ?
                (1004 - counter6000Hz) :
                (counter6000Hz - 1004);
            uint_fast16_t percent = deviation >= 99 ?
                99 :
                ((deviation * 10) / 10);
...
void __attribute__((picinterrupt(("high_priority")))) isrHighPrio(void)
{
    ...
    if (PIR4bits.CCP5IF)
    {
        ...
        if(!PORTBbits.RB0)
        {
            ...
            if (!PORTAbits.RA4 && PORTBbits.RB4)
            {
                if (!PORTAbits.RA1)
                {
                    if (++track > maxTrack)
